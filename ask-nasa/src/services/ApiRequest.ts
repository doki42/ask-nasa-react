...
export function GetPic(){
    ...
    let month: string = today.getMonth() < 9 ? "0".concat((today.getMonth()+1).toString()) : (today.getMonth()+1).toString();
    let day: string = today.getDate().toString();
    let dateMax: string = year.concat("-", month, "-", day)
    const [date, setDate] = useState<string>(dateMax);


    let urlToFetch: string = url.concat(query, apiKey, '&', 'start_date=', date, '&end_date=', date);
